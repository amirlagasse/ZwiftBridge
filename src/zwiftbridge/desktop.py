...
opens a native WebKit window on it. Closing the window shuts everything down,
including the mDNS advertisement -- leave that up and a dead "zwiftbridge"
lingers in MyWhoosh's device list.

Run with `python -m zwiftbridge.desktop`, or via zwiftbridge.app.
"""
...
def main() -> int:
    # Finder launches with cwd=/. config.toml and settings.json are resolved
    # by paths, but sit in whichever directory those chose.
    ...
    try:
        ...
        print("  .venv/bin/pip install pywebview pyobjc-framework-WebKit")
        webbrowser.open(URL)
        try:
    ...
    # ui.html would otherwise not show up until the cache aged out.
    try:
